...
def register_callbacks(app):
    ...
    def reset_rate_limits(confirm_clicks, token, role, _api_environment):
        ...
        try:
            ...
            # Debug: Print user role and token info
            print("🔍 DEBUG: Rate limit reset attempt")
            print(f"🔍 DEBUG: User role from session: {role}")
            print(f"🔍 DEBUG: Token length: {len(token) if token else 0}")
            print(f"🔍 DEBUG: Token starts with: {token[:20]}..." if token else "No token")

            # Debug: Try to decode JWT token (without verification) to see contents
            try:
                import base64
                import json
                ...
                if token and "." in token:
                    ...
                    if len(parts) >= 2:
                        ...
                        print(f"🔍 DEBUG: Token user role: {token_data.get('role', 'NOT_FOUND')}")
                        print(f"🔍 DEBUG: Token expiry: {token_data.get('exp', 'NOT_FOUND')}")
                        ...
                        if exp_timestamp:
                            ...
                            print(f"🔍 DEBUG: Token expires at: {exp_timestamp}")
                            print(f"🔍 DEBUG: Token expired: {current_time > exp_timestamp}")
            ...
            print(f"🔍 DEBUG: Reset API Response - Status Code: {resp.status_code}")
            print(f"🔍 DEBUG: Reset API Response - Headers: {dict(resp.headers)}")
            print(f"🔍 DEBUG: Reset API Response - URL: {resp.url}")

            if resp.status_code != 200:
                print(f"🔍 DEBUG: Reset Error Response Text: {resp.text[:500]}")
            ...
            # Debug: Print detailed exception information
            print(f"🔍 DEBUG: Exception in reset_rate_limits: {type(e).__name__}: {str(e)}")
            import traceback
    ...
    def fetch_rate_limit_status(_refresh_clicks, active_tab, token, role, _api_environment):
        ...
        try:
            ...
            # Debug: Print user role and token info
            print("🔍 DEBUG: Rate limit status fetch attempt")
            print(f"🔍 DEBUG: User role from session: {role}")
            print(f"🔍 DEBUG: Token length: {len(token) if token else 0}")
            print(f"🔍 DEBUG: Token starts with: {token[:20]}..." if token else "No token")
            ...
            print(f"🔍 DEBUG: API Response - URL: {resp.url}")
            ...
            # Debug: Print detailed exception information
            print(f"🔍 DEBUG: Exception in fetch_rate_limit_status: {type(e).__name__}: {str(e)}")
            import traceback
