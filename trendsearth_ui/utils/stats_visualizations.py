...
def create_user_geographic_map(user_stats_data, title_suffix=""):
    ...
    try:
        ...
        if not geographic_data:
            return html.Div(
                [
                    html.P("No geographic user data available.", className="text-muted text-center"),
                    html.Small("User location data may not be configured.", className="text-muted text-center d-block")
                ],
                className="p-4"
            )
        ...
        if not countries_data:
            return html.Div(
                [
                    ...
                    html.Small("User country information not available for this period.", className="text-muted text-center d-block")
                ],
                className="p-4"
            )
        ...
        fig = go.Figure(data=go.Choropleth(
            ...
            locationmode="ISO-3",
            colorscale="Blues",
            text=[f"{country}: {count} users" for country, count in zip(countries, user_counts)],
...
def create_execution_statistics_chart(execution_stats_data, title_suffix=""):
    ...
    try:
        ...
        if not data:
            return [html.Div(
                [
                    html.P("No execution statistics available.", className="text-muted text-center"),
                    html.Small("Execution data may not be available for this period.", className="text-muted text-center d-block")
                ],
                className="p-4"
            )]
        ...
        if task_types_data:
            ...
            fig_tasks.update_layout(
                ...
                height=max(300, len(task_names) * 30),  # Dynamic height based on number of task types
                margin={"l": 40, "r": 40, "t": 40, "b": 40}
            )
        ...
        return [html.Div(
            [
                html.P("Error creating execution statistics charts.", className="text-danger text-center"),
                html.Small(f"Error: {str(e)}", className="text-muted text-center d-block")
            ],
            className="p-4"
        )]
    ...
    try:
        ...
        if geographic_data:
            ...
            if countries_data:
                ...
                sorted_countries = sorted(countries_data.items(), key=lambda x: x[1], reverse=True)[:10]
        ...
        return [html.Div(
            [
                html.P("Error creating user statistics charts.", className="text-danger text-center"),
                html.Small(f"Error: {str(e)}", className="text-muted text-center d-block")
            ],
            className="p-4"
        )]
