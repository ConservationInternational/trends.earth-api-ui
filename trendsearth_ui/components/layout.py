...
def create_main_layout():
    ...
    container = dbc.Container(
        [
            ...
            dcc.Store(
                id="executions-table-state", storage_type="local"
            ),  # Store current sort/filter state for executions table
            dcc.Store(
                id="users-table-state", storage_type="local"
            ),  # Store current sort/filter state for users table
            dcc.Store(
                id="scripts-table-state", storage_type="local"
            ),  # Store current sort/filter state for scripts table
            dcc.Store(
                id="rate-limit-breaches-table-state", storage_type="local"
            ),  # Store current sort/filter state for rate limit breaches
            dcc.Store(
                id="executions-total-count-store", data=0, storage_type="local"
            ),  # Store total count for executions
            dcc.Store(
                id="users-total-count-store", data=0, storage_type="local"
            ),  # Store total count for users
            dcc.Store(
                id="scripts-total-count-store", data=0, storage_type="local"
            ),  # Store total count for scripts
            dcc.Store(
                id="rate-limit-breaches-total-count-store", data=0, storage_type="local"
            ),  # Store total count for rate limit breaches
            dcc.Store(id="active-tab-store", data="executions", storage_type="local"),
            dcc.Store(
