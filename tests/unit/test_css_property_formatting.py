...
class TestCSSPropertyFormatting:
    ...
    def test_login_layout_css_properties(self):
        ...
        for prop in forbidden_properties:
            assert prop not in layout_str, (
                f"Found invalid kebab-case CSS property '{prop}' - should be camelCase"
            )
    ...
    def test_no_react_error_causing_styles(self):
        ...
        assert "margin-top" not in layout_str, (
            "margin-top causes React Error #130 - use marginTop instead"
        )
        assert "min-height" not in layout_str, (
            "min-height causes React Error #130 - use minHeight instead"
        )
